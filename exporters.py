...
# Strong positive patterns that should be highlighted green
STRONG_POSITIVE_WORDS = {
    'delivered', 'achieved', 'exceeded', 'outperformed', 'strong', 'robust',
    'solid', 'growth', 'improved', 'increased', 'expanded', 'accelerated',
    'momentum', 'progress', 'success', 'successful', 'record',
}

STRONG_POSITIVE_PHRASES = [
    'ahead of expectations', 'above expectations', 'beat expectations',
    'exceeded guidance', 'raised guidance', 'strong performance',
    'record quarter', 'record results', 'all-time high',
    'double-digit growth', 'significant progress', 'meaningful improvement',
    'executed well', 'delivered results', 'on track', 'well positioned',
]

CONFIDENT_PHRASES = [
    'we delivered', 'we achieved', 'we exceeded', 'we are confident',
    'we are committed', 'we will continue', 'we remain focused',
    'our results demonstrate', 'our performance reflects',
]
...
def classify_sentence(sentence):
    ...
    # Count positives
    positive_word_count = sum(1 for w in words if w in STRONG_POSITIVE_WORDS)
    positive_phrase_count = sum(1 for phrase in STRONG_POSITIVE_PHRASES if phrase in sentence_lower)
    confident_phrase_count = sum(1 for phrase in CONFIDENT_PHRASES if phrase in sentence_lower)
    
    # Build positive annotations
    if positive_word_count >= 2:
        positives.append("Strong positive language")
    if positive_phrase_count >= 1:
        positives.append("Confident assertion")
    if confident_phrase_count >= 1:
        positives.append("First-person ownership")
    
    # Build negative annotations
    if hedge_count >= 3:
    ...
    # Determine color — prioritize negatives, but recognize positives
    if len(issues) >= 2 or hedge_count >= 4 or deception >= 2:
        return 'RED', issues
    elif len(issues) >= 1 or hedge_count >= 2:
        return 'YELLOW', issues
    elif (certainty >= 1 or positive_word_count >= 2 or positive_phrase_count >= 1 or confident_phrase_count >= 1) and hedge_count == 0:
        return 'GREEN', positives if positives else ['Strong, confident language']
    else:
        return 'NEUTRAL', []
