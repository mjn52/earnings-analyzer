...
# Import smart Q&A module (improved version)
from smart_qa import analyze_script_for_qa, filter_operator_content
    ...
    with col2:
        if st.button("🚀 Analyze Script", use_container_width=True, type="primary"):
            if len(final_text) >= 100:
                # Filter out operator/moderator content before analysis
                filtered_text = filter_operator_content(final_text)
                st.session_state.text = filtered_text
                st.session_state.original_text = final_text
                
                with st.spinner("Analyzing your script..."):
                    # Core analysis (on filtered text)
                    st.session_state.analysis = analyze_transcript(filtered_text, lm_dict)
                    st.session_state.legal_analysis = analyze_with_legal_context(
                        filtered_text, analyze_transcript, lm_dict
                    )
                    
                    # Smart Q&A analysis
                    st.session_state.smart_qa = analyze_script_for_qa(filtered_text)
                    
                    # Legacy advanced analyses (keeping for compatibility)
                    analyst_questions = generate_analyst_questions(filtered_text, st.session_state.analysis)
...
# ============================================================
    ...
    # Analyst Q&A Section (Smart Q&A)
    if 'smart_qa' in st.session_state and st.session_state.smart_qa:
        smart_qa = st.session_state.smart_qa
        st.subheader(f"🎤 Likely Analyst Questions ({smart_qa['total_questions']} predicted)")
        
        # Show high confidence count
        st.caption(f"🔴 {smart_qa['high_confidence_count']} high-probability questions")
        
        questions = smart_qa['questions']
        answers = smart_qa['answers']
        
        for i, (q, a) in enumerate(zip(questions[:7], answers[:7]), 1):
            confidence_pct = int(q.confidence * 100)
            confidence_icon = "🔴" if confidence_pct >= 80 else "🟡" if confidence_pct >= 60 else "🟢"
            
            with st.expander(f"{confidence_icon} Q{i}: {q.question}", expanded=(i <= 2)):
                st.caption(f"Category: {q.category.replace('_', ' ').title()} | Confidence: {confidence_pct}%")
                
                if q.trigger_context:
                    st.markdown(f"**Trigger:** _{q.trigger_context[:150]}..._")
                
                st.markdown("---")
                st.markdown(f"**📋 Proposed Response ({a.answer_strategy}):**")
                st.write(a.answer)
                
                if a.supporting_data:
                    st.markdown(f"**📊 Key Data Points:** {', '.join(a.supporting_data[:3])}")
                
                if a.confidence_notes:
                    for note in a.confidence_notes:
                        st.caption(f"💡 {note}")
